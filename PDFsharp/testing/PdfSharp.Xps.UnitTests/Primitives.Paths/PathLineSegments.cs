...
namespace PdfSharp.Xps.UnitTests.Primitives.Paths
{
  ...
  /// </summary>
  [TestClass]
  public class PathsLineSegments : TestBase
  {
    ...
    [TestMethod]
    public void TestPathLineSegments()
    {
      RenderVisual("LineSegments", CreateLineSegments);
    }

    Visual CreateLineSegments()
    {
      DrawingContext dc;
      DrawingVisual dv = PrepareDrawingVisual(out dc);

      Point startPoint = new Point(20, 120);
      Point[] points = { new Point(80, 10), new Point(210, 60) };
      PathGeometry path;
      PathFigure figure;
